...
const CONTRACT_ADDRESS = import.meta.env.VITE_BOUNTY_REGISTRY;
...
export default function AdminPage() {
  ...
  const [donorContributions, setDonorContributions] = useState<Record<string, Record<string, string>>>({});
  ...
  const fetchBounties = async () => {
    ...
    try {
      const rawBounties: any[] = await contract.getAllBounties();
      const loadedBounties: Bounty[] = rawBounties.map(b => ({
        id: b.id.toString(),
        ngo: b.ngo,
        active: b.active,
      ...
      const contributionMap: Record<string, Record<string, string>> = {};

      for (const b of loadedBounties) {
        const ms = await contract.getAllMilestones(b.id);
        milestoneMap[b.id] = ms.map((m: any) => ({
  ...
  const approveMilestone = async (bountyId: string, milestoneId: number) => {
    if (!contract) return;
    try {
  ...
  return (
    <div className="container">
      <style>{`
        ...
        p { margin: 0.3rem 0; }
        button { cursor: pointer; border: none; border-radius: 4px; padding: 0.4rem 0.8rem; margin: 0.2rem 0; font-size: 0.9rem; }
        .btn-connect { background: #000; color: #fff; }
        .btn-accent { background: #0070f3; color: #fff; }
        .btn-admin { background: #800080; color: #fff; }
        .bounty-card { border: 1px solid #000; padding: 0.8rem; margin-bottom: 1rem; border-radius: 6px; }
        .milestone { display: flex; justify-content: space-between; align-items: center; margin-top: 0.3rem; }
      `}</style>
      ...
      {!address ? (
        ...
        <>
          ...
          {userRoles.length > 0 ? (
            <p style={{ color: "green", fontWeight: "bold" }}>Roles: {userRoles.join(", ")}</p>
          ) : (
        ...
        <div key={b.id} className="bounty-card">
          ...
          {milestones[b.id]?.map((m, idx) => (
            <div key={idx} className="milestone">
              <span>
                Milestone {idx} — Amount: {m.amount} HAKI — Approved: {m.approved ? "✅" : "❌"} — Released: {m.released ? "✅" : "❌"}
              </span>
              {!m.approved &&
                (userRoles.includes("CASE_STEWARD_ROLE") || userRoles.includes("ADMIN_ROLE")) && (
                  <button onClick={() => approveMilestone(b.id, idx)} className="btn-accent">
                    Approve
                  </button>
                )}
            </div>
